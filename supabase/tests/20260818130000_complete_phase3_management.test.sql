...
insert into phase3_completion_tap_output(output) select plan(27);
...
insert into phase3_completion_tap_output(output) select lives_ok($$select public.register_sport_league_player(
  (select id from phase3_completion_ids where kind = 'competition'), (select id from public.sport_profiles where account_id = 'f4350000-0000-0000-0000-000000000002')
)$$, 'the first player occupies a capacity place');
insert into phase3_completion_tap_output(output) select lives_ok($$select public.register_sport_league_player(
  (select id from phase3_completion_ids where kind = 'competition'), (select id from public.sport_profiles where account_id = 'f4350000-0000-0000-0000-000000000003')
...
insert into phase3_completion_tap_output(output) select throws_ok($$select public.register_sport_league_player(
  (select id from phase3_completion_ids where kind = 'competition'), (select id from public.sport_profiles where account_id = 'f4350000-0000-0000-0000-000000000004')
...
insert into phase3_completion_tap_output(output) select lives_ok($$select public.reorder_sport_fixtures(
  ...
  (select array_agg(id order by display_order desc) from public.sport_fixtures), 2
)$$, 'the full fixture set can be reordered atomically');
insert into phase3_completion_tap_output(output) select is(
  (select court from public.sport_fixtures order by display_order limit 1), 'Court 2', 'the requested manual order is persisted');
...
insert into phase3_completion_tap_output(output) select lives_ok($$select public.assign_sport_fixture_official(
  (select id from public.sport_fixtures order by display_order limit 1), 'f4350000-0000-0000-0000-000000000005', 'SCOREKEEPER'
)$$, 'an account-backed scorekeeper can be assigned');
insert into phase3_completion_tap_output(output) select is((select count(*) from public.sport_fixture_officials), 1::bigint, 'one official assignment is stored');
...
insert into phase3_completion_tap_output(output) select is(
  (select name from public.sport_competition_venues order by display_order limit 1),
