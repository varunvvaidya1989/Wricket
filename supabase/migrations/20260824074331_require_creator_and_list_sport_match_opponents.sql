-- Let an active player discover other SportStage accounts that selected the
-- same sport. A blank query provides a small initial opponent list; non-blank
-- searches still require at least two characters.
...
declare
  ...
  clean_query text := pg_catalog.btrim(pg_catalog.coalesce(p_query, ''));
begin
  ...
  order by
    ...
    profile.id
  limit pg_catalog.least(pg_catalog.greatest(pg_catalog.coalesce(p_limit, 20), 1), 40);
end;
$$;

-- A standalone match created from a user's account must include that user as
-- one of its players. This prevents creating matches solely between others.
...
begin
  ...
  insert into public.sport_scoring_matches(
    ...
    selected_sport.id,
    normalized_format,
    pg_catalog.coalesce(side_a_names, '[]'::jsonb),
    pg_catalog.coalesce(side_b_names, '[]'::jsonb),
    p_rules_snapshot,
    (select auth.uid())
